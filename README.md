...
- **📊 고도화된 당첨 확률 예측** — EWMA, 수요 분산 컨텍스트 파악, 몬테카를로 시뮬레이션을 활용한 최적화 전략 제공.
- **실시간 대시보드** — GitHub Pages를 통한 공고 이력, 진행 중인 모집(D-day), 예측 모델 인사이트 확인.
- **세련된 초대장** — 친구에게 바이럴 느낌의 깔끔한 웹 초대장 제공 및 독립적인 권한 관리(프라이버시 보호).
...
## 데이터 분석 및 당첨 예측 모델 (`analyzer.py`)

단순한 공고 알림을 넘어, 과거 데이터를 기반으로 **최적의 지원 전략**을 수립할 수 있도록 고도화된 통계 분석 모델을 제공합니다.

- **지수가중이동평균(EWMA) 예측**: 과거 경쟁률 이력에 최신 가중치를 부여하여 최근 추세를 반영한 실질 경쟁률을 예측합니다.
- **수요 분산 컨텍스트(Demand Context) 분석**: 모집 공고에 단독 타입만 나올 때(독식)와 여러 타입이 동시 모집될 때(수요 분산)의 지원자 쏠림 비율을 정규화하여 경쟁률 예측을 정교하게 보정합니다.
- **몬테카를로 시뮬레이션(Monte Carlo)**: 예측 경쟁률의 불확실성을 삼각분포로 샘플링하여 10,000회 시뮬레이션을 수행, 'N회 지원 내 당첨될 확률'과 같은 누적 성공 기대값을 도출합니다.
- **실질 당첨 확률 산출**: 예비 번호 선발 배수(기본 3배수)와 실제 예비 번호 전환율을 반영하여 직접 당첨 확률과 예비 당첨 확률을 합산한 실질 당첨 확률을 제공합니다.

분석된 인사이트(가장 경쟁이 낮을 것으로 예상되는 추천 타입, 추세 경고 등)는 대시보드에서 직관적으로 확인할 수 있습니다.
...
## 향후 계획 (TODO)
- **대시보드 시각화 강화**: 현재 HTML 기반 대시보드에 Chart.js 등을 연동하여 경쟁률 추세 그래프 추가.
- **다중 사용자 지원 (Multi-user)**: 2인(본인+친구) 제한 구조에서 발전하여 DB(Supabase/Firebase 등) 연동을 통한 다수 구독자 관리 시스템 도입.
- **데이터 스토리지 분리**: `history.json` 데이터가 커질 것을 대비해 GitHub 커밋 저장 방식에서 외부 스토리지(S3 등)로 분리.
- **테스트 커버리지 확대**: 알림 파이프라인 외에 경쟁률 예측 로직에 대한 단위 테스트(Unit Test) 추가.
